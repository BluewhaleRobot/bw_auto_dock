#ifndef LOCAL_GRID_H_
#define LOCAL_GRID_H_
...
public:
  LocalGrid(float resolution,int half_length,std::string frame_id)
  {
    ...
    for (int i=0; i<l5.num_points; i++)
    {
      ...
      {
        if(l5.points[i].y == p0.y) continue;
      ...
      {
        if(l4.points[i].y == p0.y) continue;
        x1 = (j-p0.x)*resolution_ ;
        if(j==p0.x)
        {
          theta = PI/2.0;
      ...
      {
        if(l3.points[i].y == p0.y) continue;
        x1 = (j-p0.x)*resolution_ ;
        if(j==p0.x)
        {
          theta = PI/2.0;
    ...
    pub_occupancy_grid_ = nodeHandler_.advertise<nav_msgs::OccupancyGrid>("bw_auto_dock/dock_occupancy_map", 1,true);
  ...
  {
    boost::mutex::scoped_lock lock(mMutex_);
    origin_[0] = center[0] - ((half_length_-0.5)*resolution_)*cos(center[2]) +  ((half_length_-0.5)*resolution_)*sin(center[2]);
    origin_[1] = center[1] - ((half_length_-0.5)*resolution_)*sin(center[2]) - ((half_length_-0.5)*resolution_)*cos(center[2]);
    origin_[2] = center[2];
    grid_ready_ = true;
  ...
  void update_clear(const float (&pose)[3])
  {
    ...
    if(x_num>(side_num_-1) || x_num<0) return;
    if(y_num>(side_num_-1) || y_num<0) return;

    int pose_index = x_num + y_num*side_num_;

    if(pose_index == pose_index_last) return;
    ...
    for(int i=0;i<index_clear_;i++)
    {
      ...
      x=robot_clear_points_[i][0]*cos_theta - robot_clear_points_[i][1]*sin_theta + pose[0]; //转换到frame_id坐标系
      ...
      //ROS_INFO("update_clear2  %f %f %f %f %d %d %d",robot_clear_points_[i][0],robot_clear_points_[i][1],x,y,x_num,y_num,i);

      if(x_num>(side_num_-1)||x_num<0) continue;
      if(y_num>(side_num_-1)||y_num<0) continue;
  ...
  void update_sensor(const DOCK_POSITION dock_position_current,const float (&pose)[3])
  {
    ...
    switch (dock_position_current) {
      ...
      case DOCK_POSITION::left:
        if((left_sensor_cells[pose_index]&4) == 4) return;
      ...
      case DOCK_POSITION::left_center:
        if((left_sensor_cells[pose_index]&8) == 8) return;
      ...
      case DOCK_POSITION::left_up:
        if((left_sensor_cells[pose_index]&1) == 1) return;
      ...
      case DOCK_POSITION::left_down:
        if((left_sensor_cells[pose_index]&2) == 2) return;
        ...
        case DOCK_POSITION::right:
          if((right_sensor_cells[pose_index]&4) == 4) return;
        ...
        case DOCK_POSITION::right_center:
          if((right_sensor_cells[pose_index]&8) == 8) return;
        ...
        case DOCK_POSITION::right_down:
          if((right_sensor_cells[pose_index]&1) == 1) return;
        ...
        case DOCK_POSITION::right_up:
          if((right_sensor_cells[pose_index]&2) == 2) return;
          ...
          case DOCK_POSITION::back:
            if((back_sensor_cells[pose_index]&4) == 4) return;
          ...
          case DOCK_POSITION::back_center:
            if((back_sensor_cells[pose_index]&8) == 8) return;
          ...
          case DOCK_POSITION::back_left:
            if((back_sensor_cells[pose_index]&1) == 1) return;
          ...
          case DOCK_POSITION::back_right:
            if((back_sensor_cells[pose_index]&2) == 2) return;
    ...
    cos_theta0 = cos(origin_[2]);
    sin_theta0 = sin(origin_[2]);
    //ROS_INFO("update_sensor1 %d %f %f %f %f %f %d %d %d",(int)dock_position_current,pose[0],pose[1],pose[2],origin_[0],origin_[1],x_num,y_num,index);
    ...
    for(int i=0;i<index;i++)
    {
      ...
      if(robot_clear_cells[pose_index] == 1 ) continue;//清除已使能,robot可以到达所以肯定没有充电桩
  ...
  bool get_dock_position(float (&pose)[3])
  {
    boost::mutex::scoped_lock lock(mMutex_);
    if(!grid_ready_) return false;
    if(!enforce_dock_position_) this ->update_mean();
  ...
  void update_mean(void)
  {
    ...
    for(int i=0;i<total_cell_num_;i++)
    {
      ...
      else
      {
        ...
        if(value_temp>99) value_temp=101;

...
void StatusPublisher::Refresh()
{
    ...
    if(mbUpdated)
    {
      ...
      //根据4个红外接收器状态判断dock方位
      if((sensor_status.left_sensor1+sensor_status.left_sensor2+sensor_status.right_sensor2+sensor_status.right_sensor1)==0)
      {
        //没有发现
        mdock_position_ = DOCK_POSITION::not_found;
      }
          ...
          else
          {
            ...
            switch (sensor_status.left_sensor2){
              case 1:
                ...
                break;
              case 2:
                if(sensor_status.right_sensor2 != 4  && sensor_status.right_sensor2 !=0 && sensor_status.right_sensor2 !=2 && sensor_status.right_sensor2 !=6)
                {
                  mdock_position_ = DOCK_POSITION::back_center;
                }
                else

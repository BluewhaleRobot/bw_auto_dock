...
DockControler::DockControler(double back_distance,double max_linearspeed,double max_rotspeed,StatusPublisher* bw_status,CallbackAsyncSerial* cmd_serial)
{
...
void DockControler::run()
{
    ...
    ros::Subscriber sub1 = nodeHandler.subscribe("/bw_auto_dock/EnableCharge", 1, &DockControler::updateChargeFlag,this);
    ros::Subscriber sub2 = nodeHandler.subscribe("/odom", 1, &DockControler::updateOdom,this);
    ros::spin();
}

void DockControler::updateChargeFlag(const std_msgs::Bool& currentFlag)
{
...
void DockControler::updateOdom(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  if(mcurrentChargeFlag_)
  {
    ...
    switch (mcharge_status_temp_) {
      ...
      case CHARGE_STATUS_TEMP::finding1:
        ...
        if(dock_position_current==DOCK_POSITION::left_center || dock_position_current==DOCK_POSITION::right_center)
        {
        ...
        if(dock_position_current == DOCK_POSITION::back_center && (bw_status_->sensor_status.left_sensor2==3||bw_status_->sensor_status.left_sensor2==7)&&(bw_status_->sensor_status.right_sensor2==3||bw_status_->sensor_status.right_sensor2==7))
        {
          mPose1_ = mRobot_pose_;
      ...
      case CHARGE_STATUS_TEMP::finding4:
        {
          ...
          //if(dock_position_current == DOCK_POSITION::back_center && (((bw_status_->sensor_status.left_sensor2==3||bw_status_->sensor_status.left_sensor2==7) && mdock__referenss_position_ == DOCK_POSITION::right_center )||((bw_status_->sensor_status.right_sensor2==3 ||bw_status_->sensor_status.right_sensor2==7)  && mdock__referenss_position_ == DOCK_POSITION::left_center)) )
          //if(((bw_status_->sensor_status.left_sensor2==3||bw_status_->sensor_status.left_sensor2==7) && mdock__referenss_position_ == DOCK_POSITION::right_center )||((bw_status_->sensor_status.right_sensor2==3 ||bw_status_->sensor_status.right_sensor2==7)  && mdock__referenss_position_ == DOCK_POSITION::left_center) )
          if(dock_position_current == DOCK_POSITION::back_center)
          {
        ...
        else
        {
          ...
          if(unusefull_num_>20)
          {
            ...
            if((current_average_)<0.1)
            {
              //进入充满状态
              //下发充满显示状态使能命令，绿灯
              char cmd_str[6]={(char)0xcd,(char)0xeb,(char)0xd7,(char)0x02,(char)0x4B,(char)0x02};
              if(NULL!=mcmd_serial_)
              {
                  mcmd_serial_->write(cmd_str,6);
              }
...
void DockControler::caculatePose3()
{
  ...
  mPose3_[2] = theta;
  ROS_DEBUG("theta12  %f  %f x1 y1 %f %f x2 y2 %f %f ",theta1,theta2,mPose1_.position.x,mPose1_.position.y,mPose2_.position.x,mPose2_.position.y);

}

bool DockControler::backToPose3()
{
  geometry_msgs::Pose current_pose = mRobot_pose_;
  ...
  ROS_DEBUG("theta3  %f  %f x3 y3 %f %f x y %f %f x2 y2 %f %f ",mPose3_[2],theta,mPose3_[0],mPose3_[1],x,y,x2,y2);

  if(fabs(x2)<=0.03) return true;
  geometry_msgs::Twist current_vel;
  current_vel.linear.x = -0.1;
  current_vel.linear.y = 0;
  current_vel.linear.z = 0;
...
bool DockControler::backToDock()
{
  ...
  theta = yaw;
  //如果侦测到电压，停止
  if(bw_status_->sensor_status.power>9.0) return true;

  //如果侦测到已进入死角，停止
  if(bw_status_->sensor_status.left_switch1==1||bw_status_->sensor_status.right_switch1==1) return true;
  ...
  ROS_DEBUG("ousp1 delta  %f  error %f %f %f sonsor %d ",rot_delta,left2_error1_,rot_error_temp1,rot_error_temp2,bw_status_->sensor_status.left_sensor2);
  ...
  // ROS_DEBUG("ousp2 delta  %f  error %f %f %f sonsor %d ",rot_delta,right2_error1_,rot_error_temp1,rot_error_temp2,bw_status_->sensor_status.right_sensor2);
  if(rot_delta>max_rotspeed_) rot_delta = max_rotspeed_;
  if(rot_delta<-max_rotspeed_) rot_delta = -max_rotspeed_;
  rot_z_ = rot_delta;
...
float DockControler::computeDockError()
{
  ...
  if(l2==0)
  {
    if(r2==0) return return_value;
    return_value = 1.0;
  }
  else if(r2==0)
  {
    if(l2==0) return return_value;
    return_value = -1.0;
  }
  else
  {
    switch (r2) {
      case 1:
      case 5:
        if(l2==2||l2==3||l2==6||l2==7) return return_value;
        return_value = 1.0;
        break;
      case 2:
      case 3:
      case 6:
      case 7:
        if(l2==1||l2==3||l2==5||l2==7) return return_value;
        if(l2==2||l2==6) return_value = -1.0;
        if(l2==4) return_value = 1.0;
        break;
      case 4:
        if(l2==4) return return_value;
        return_value = -1.0;
        break;
    }
  }
...
void DockControler::setDockPid(double kp,double ki,double kd)
{
  kp_ = kp;
  ki_ = ki;
  kd_ = kd;
...
bool DockControler::rotateOrigin()
{
  geometry_msgs::Pose current_pose = mRobot_pose_;
  float x,y,theta;
  x = current_pose.position.x;
...
void DockControler::caculatePose4()
{
...
geometry_msgs::Pose DockControler::getRobotPose()
{
  boost::mutex::scoped_lock lock(mMutex_pose);
  return mRobot_pose_;
}
bool DockControler::getRobotPose(float (&robot_pose)[3])
{
  boost::mutex::scoped_lock lock(mMutex_pose);
  if(!mPose_flag_) return false;
  geometry_msgs::Pose current_pose = mRobot_pose_;
...
bool DockControler::getIRPose(float (&robot_pose)[3])
{
  boost::mutex::scoped_lock lock(mMutex_pose);
  if(!mPose_flag_) return false;
  geometry_msgs::Pose current_pose = mRobot_pose_;
...
void DockControler::setDockPositionCaculate(CaculateDockPosition * dock_position_caculate)
{
  mdock_position_caculate_ = dock_position_caculate;
}

void DockControler::caculateStation3()
{
  //选择距离远的点
  geometry_msgs::Pose current_pose = mRobot_pose_;
  float x,y,theta;
...
//  ROS_INFO("station3 %f %f %f %f %f %f %f %f %f",mstationPose1_[0],mstationPose1_[1],mstationPose2_[0],mstationPose2_[1],mstationPose3_[0],mstationPose3_[1],mstationPose3_[2],x,y);
}

bool DockControler::rotate2Station3()
{
  //旋转到正对目标点3
  geometry_msgs::Pose current_pose = mRobot_pose_;
...
bool DockControler::goToStation3()
{
  geometry_msgs::Pose current_pose = mRobot_pose_;
  float x,y,theta,x2,y2;
  x = current_pose.position.x;
